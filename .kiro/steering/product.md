...
- KI-Autofill: optional AI-powered plant data generation when adding/editing plants (description, events, category, etc.) — requires `ANTHROPIC_API_KEY` or `MISTRAL_API_KEY`
- Monthly garden report: automated email on the 1st of each month with all upcoming tasks, blooms, harvests etc., generated by Claude (Anthropic API) and sent via Gmail SMTP
...
- Monthly report via cron job (`0 8 1 * *`) calling `monthly_report.py` inside the container
- Outbound SMTP (Gmail) and HTTPS (Anthropic API) from the container — no inbound ports needed beyond the web UI
